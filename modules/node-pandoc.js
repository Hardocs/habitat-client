...
const nodePandoc = function() {
  ...
  onStdErrData = function (err) {
    errResult += err;
    // console.log('onStdErrData:err: ' + err);
    // console.log('onStdErrData:err: ' + err)
    // callback(new Error(err));
  };

  onStdErrEnd = function () {
    if (errResult) {
      // console.log ('onStdErrEnd: ' + errResult);
      callback(new Error(errResult));
    }
  };
  ...
  onStatCheck = function (err, stats) {
    ...
    pdSpawn.stderr.on('end', onStdErrEnd);
  };
